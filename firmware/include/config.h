...
// ── TOF display ───────────────────────────────────────────────────────────
constexpr int16_t TOF_MAX_MM =
    800; // 100 % distance for OLED percentage display

// ── Enemy detection ────────────────────────────────────────────────────────
constexpr int16_t ENEMY_ANY_MM = 800; // any TOF < this → enemy visible
constexpr int16_t ENEMY_FRONT_MM =
    600; // both front TOFs < this → attack straight

// ── Start maneuver ─────────────────────────────────────────────────────────
constexpr float START_FWD_SPEED =
    0.75f; // forward speed during slight-turn start
constexpr float START_HARD_TURN = 1.0f; // outer wheel speed for hard pivot
constexpr float START_SLIGHT_BIAS =
    0.35f; // inner wheel reduction for slight curve
